...
func Test_AggregateBase_BuildFromHistory_should_transition_and_set_id_version(t *testing.T) {
	p := aggAgg{}
	p.TrackChange(&p, &born{Name: "Something"})
	p.TrackChange(&p, &born{Name: "woo"})
	...
	assert.Equal(t, "woo", p1.name)
	assert.Equal(t, 1, p1.age)
	assert.Equal(t, Version(3), p1.Version())
	assert.False(t, p1.HasUnsavedEvents())
}

func Test_AggregateBase_SetID_should_return_error_when_id_is_already_set(t *testing.T) {
	p := aggAgg{
		AggregateBase: AggregateBase{id: "happy"},
	}

	assert.ErrorIs(t, p.SetID("nope"), ErrAggregateAlreadyExists)
}

func Test_AggregateBase_SetID_should_change_the_id(t *testing.T) {
	p := aggAgg{}
	assert.NoError(t, p.SetID("yes"))
	assert.Equal(t, "yes", p.ID())
}

func Test_AggregateBase_Root_should_return_pointer_to_self(t *testing.T) {
	p := aggAgg{}
	assert.Equal(t, &p.AggregateBase, p.Root())
}

func Test_AggregateBase_setInternals_should_set_id_version_and_clear_events(t *testing.T) {
	p := aggAgg{}
...
func Test_AggregateBase_update_should_set_version_to_last_event_and_clear_events(t *testing.T) {
	p := aggAgg{}
	p.version = 20
	p.update()
...
func Test_AggregateBase_path_should_return_path(t *testing.T) {
	p := aggAgg{}
